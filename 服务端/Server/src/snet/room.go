...
func NewRoom(value int32) *Room {
	a := &Room{
		...
		CanDoHandle:     true,
		TaskQueue:       make(chan *Request, Settings.GlobalObject.WorkerPoolSize),
...
//对该房间广播
func (r Room) BroadRoom(message *Message) {
...
func (g *GameReady) Handle(request *Request) {
	//fmt.Println("Ready!!")
...
func (g *GameDoing) Handle(request *Request) {
	//fmt.Println("Doing!!")
	rom := &Data.PlayerInfo{}
...
//游戏结束时候执行的Handle
func (g *GameEnd) Handle(request *Request) {
	fmt.Println("Over!!")
	msg := &Data.PlayerInfo{
		Roomid:   0,
		Uid:      0,
