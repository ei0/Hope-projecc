...
type MsgHandle struct {
	//存放Handle的Map
	HandleMap map[uint32]IRouter

	//负责Worker取任务的消息队列
	MessageQueue []chan *Request
...
func NewMsgHandle() *MsgHandle {
	return &MsgHandle{
		HandleMap:       make(map[uint32]IRouter),
		WorkerPoolSize:  Settings.GlobalObject.WorkerPoolSize,
		MessageQueue:    make([]chan *Request, Settings.GlobalObject.WorkerPoolSize),
		IsRoomHandleMap: make(map[uint32]bool),
	}
}

//处理消息
func (msgh *MsgHandle) DoMsgHandler(request *Request) {
...
func (msgh *MsgHandle) AddRouter(msgId uint32, router IRouter, detail string, value int32) {
	//判断当前msgid是否绑定了处理方法
...
func (msgh *MsgHandle) StartWorkerPool() {
	...
	for i := 0; i < int(msgh.WorkerPoolSize); i++ {
		...
		msgh.MessageQueue[i] = make(chan *Request, Settings.GlobalObject.MaxWorkerSize)
...
//启动一个Worker工作流程
func (msgh *MsgHandle) StartOneWorker(workerId int32, taskQueue chan *Request) {
...
func (msgh *MsgHandle) SendMsgToMesQueue(request *Request) {
...
func (msgh *MsgHandle) ReturnChannel(request *Request) chan *Request {
	//说明是房间方法，那么返回房间号的管道
